...
def test_mettre_a_jour_tout_joueurs_et_matchs(tmp_path: Path) -> None:
    ...
    nouveaux_joueurs.write_text(
        "player_name,age,taille\nNadal,37,185\nAlcaraz,20,183\n"
    )
...
def test_editer_joueur_manuellement_succes(
    mock_input: MagicMock, tmp_path: Path
) -> None:
...
@patch("builtins.input")
def test_editer_equipe_manuellement_succes(
    mock_input: MagicMock, tmp_path: Path
) -> None:
    ...
    fichier_csv.write_text(
        "team_id,team_name,stade\n101,Paris SG,Parc\n102,Marseille,Velodrome\n"
    )
