...
def test_trouver_nom_equipe_par_nom_direct() -> None:
    ...
    resultat = formatter._trouver_nom_equipe(noms_possibles, ids_possibles, "Equipe Defaut")
    assert resultat == "Fnatic"


def test_trouver_nom_equipe_par_id_croise() -> None:
    """Cas 2: Pas de nom dans le match, on utilise l'ID pour chercher dans liste_equipes
    ."""
    faux_match = SimpleNamespace(home_team_api_id="9987", sport="football", season="2023")
...
def test_trouver_nom_equipe_ignore_les_nan() -> None:
    ...
    """
    faux_match = SimpleNamespace(home_team_name="nan", home_team_api_id="None", sport="tennis")
    formatter = MatchFormatter(match=faux_match)
    
    resultat = formatter._trouver_nom_equipe(["home_team_name"], ["home_team_api_id"], "Equipe Inconnue")
