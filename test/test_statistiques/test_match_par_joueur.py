...
def test_stats_sport_a_points_et_nuls() -> None:
    ...
    match_1 = SimpleNamespace(list_home_player=["Lebron James"], 
                              list_away_player=["Curry"], home_team_score = 110, 
                              away_team_score = 100)
    match_2 = SimpleNamespace(list_home_player=["Durant"], 
                              list_away_player=["Lebron James"], home_team_score = 110, 
                              away_team_score = 100)
    match_3 = SimpleNamespace(list_home_player=["Lebron James"], 
                              list_away_player=["Jokic"], home_team_goal = 100, 
                              away_team_goal = 100)
...
def test_stats_tennis_et_lol() -> None:
    ...
    #tennis
    match_tennis_1 = SimpleNamespace(list_home_player=["Nadal"], 
                                     list_away_player=["Federer"]) 
    match_tennis_2 = SimpleNamespace(list_home_player=["Djoko"], 
                                     list_away_player=["Nadal"])   
    
    stats_tennis = calculer_stats_joueur("Nadal", "tennis", 
                                         [match_tennis_1, match_tennis_2])
