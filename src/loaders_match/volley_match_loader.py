...
class VolleyMatchLoaderFemme:
    ...
    def load_all_match(self):
        ...
        for i in range(len(df_match_volley_femme)):
            ...
            match = Match(None, "volley", None, None)
            match.id = f"F{i+1}"
...
class VolleyMatchLoaderHomme:
    ...
    def load_all_match(self):
        ...
        for i in range(len(df_match_volley_homme)):
            ...
            if code_1:
                match.list_home_player = df_player_homme[df_player_homme["country_code"] == code_1]["name"].tolist()
            else:
                match.list_home_player = []
                print(f"Erreur : Le pays {pays_1} n'a pas été trouvé dans le dictionnaire.")
                
            if code_2:
                match.list_away_player = df_player_homme[df_player_homme["country_code"] == code_2]["name"].tolist()
            else:
                match.list_away_player = []
                print(f"Erreur : Le pays {pays_2} n'a pas été trouvé dans le dictionnaire.")
...
class VolleyMatchLoader:
    ...
    def load_all_match(self):
        ...
        chemin_match_f = "/home/onyxia/work/Projet-1A-2026/data/volleyball_tdd/match_women.csv"
        chemin_player_f = "/home/onyxia/work/Projet-1A-2026/data/volleyball_tdd/player_women.csv"
        
        chemin_match_h = "/home/onyxia/work/Projet-1A-2026/data/volleyball_tdd/match_men.csv"
        chemin_player_h = "/home/onyxia/work/Projet-1A-2026/data/volleyball_tdd/player_men.csv"

        mapping_femmes = {
            "France": "FRA", "Türkiye": "TUR", "Kenya": "KEN", "Dominican Republic": "DOM",
            "China": "CHN", "United States": "USA", "Japan": "JPN", "Italy": "ITA",
            "Poland": "POL", "Serbia": "SRB", "Brazil": "BRA", "Netherlands": "NED"
        }
        
        mapping_hommes = {
            "France": "FRA", "Poland": "POL", "Slovenia": "SLO", "Italy": "ITA",
            "United States": "USA", "Brazil": "BRA", "Japan": "JPN", "Germany": "GER",
            "Argentina": "ARG", "Serbia": "SRB", "Canada": "CAN", "Egypt": "EGY"
        } 

        loader_femme = VolleyMatchLoader(chemin_match_f, chemin_player_f, mapping_femmes)
        liste_matchs_femmes = loader_femme.load_all_matches()

        loader_homme = VolleyMatchLoader(chemin_match_h, chemin_player_h, mapping_hommes)
