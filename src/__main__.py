        ...
        while True:
            ...
            if choix_action == "1":
                try:
                    saisons_dispo = sorted(
                        list(
                            set(
                                str(getattr(m, "season", ""))
                                for m in matchs
                                if getattr(m, "season", None)
                            )
                        )
                    )
                    saison_choisie = None
                    ...
                    nom_e1 = (
                        input("Entrez le nom de la première équipe : ").strip().lower()
                    )
                    nom_e2 = (
                        input("Entrez le nom de la deuxième équipe : ").strip().lower()
                    )
                    ...
                    for m in matchs:
                        if (
                            saison_choisie
                            and str(getattr(m, "season", "")) != saison_choisie
                        ):
                            continue

                        m_h_id = str(
                            getattr(
                                m, "home_team_api_id", getattr(m, "team_id_home", "")
                            )
                        )
                        m_a_id = str(
                            getattr(
                                m, "away_team_api_id", getattr(m, "team_id_away", "")
                            )
                        )
                        ...
                        match_ok = False
                        if id_e1 and id_e2:
                            if (m_h_id == id_e1 and m_a_id == id_e2) or (
                                m_h_id == id_e2 and m_a_id == id_e1
                            ):
                                match_ok = True
                        elif (nom_e1 in m_h_name and nom_e2 in m_a_name) or (
                            nom_e2 in m_h_name and nom_e1 in m_a_name
                        ):
                            match_ok = True
                    ...
                    if not matchs_trouves:
                        ...
                        for m_trouve in matchs_trouves:
                            formateur = MatchFormatter(
                                m_trouve, toutes_les_equipes[nom_sport_choisi]
                            )
            ...
            # --- ACTION 2 : STATISTIQUES ÉQUIPE ---
                ...
                if nom_sport_choisi == "volley":
                    print("\n1. Hommes\n2. Femmes")
                    genre_choisi = (
                        "Homme" if input("Catégorie (1 ou 2) : ") == "1" else "Femme"
                    )
                ...
                try:
                    ...
                    resultat = calculateur.get_team_points(
                        nom_equipe, saison_choisie, genre=genre_choisi
                    )
            ...
            # --- ACTION 4 :
                ...
                if choix_comp == "1":
                        ...
                        if choix_s != "0" and choix_s.strip():
                            try:
                                ...
                                matchs_filtres = [
                                    m
                                    for m in matchs
                                    if str(getattr(m, "season", "")) == saison_choisie
                                ]
                            except Exception:
                    ...
                    try:
                        res_e1 = calculateur.get_team_points(
                            nom_e1, saison_choisie, genre=genre_choisi
                        )
                        res_e2 = calculateur.get_team_points(
                            nom_e2, saison_choisie, genre=genre_choisi
                        )

                        if isinstance(res_e1, str):
                            print(f"\n❌ Erreur Équipe 1 : {res_e1}")
                        elif isinstance(res_e2, str):
