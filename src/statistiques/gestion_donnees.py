...
class DataUpdater:
    ...
    def mettre_a_jour_tout(
        self, chemin_nouveaux_matchs: str, chemin_nouveaux_joueurs: str | None = None
    ) -> None:
    ...
    def _update_player_characteristics(self, path_updates: str) -> None:
        ...
        print(
            "✅ Caractéristiques des joueurs mises à jour "
    ...
    def editer_joueur_manuellement(self) -> bool:
        ...
        colonnes_noms = [
            col for col in df.columns if "name" in col.lower() or "nom" in col.lower()
        ]
        ...
        nom_recherche = input(
            "\nEntrez le nom (ou une partie du nom) du joueur à modifier : "
        ).strip()

        masque = pd.Series(False, index=df.index)
        for col in colonnes_noms:
            masque = masque | df[col].astype(str).str.contains(
                nom_recherche, case=False, na=False
            )
    ...
    def editer_equipe_manuellement(self) -> bool:
        ...
        nom_recherche = input(
            "\nEntrez le nom (ou une partie du nom) de l'équipe à modifier : "
        ).strip()

        noms_complets_virtuels = df[colonnes_noms].astype(str).agg(" ".join, axis=1)

        masque = noms_complets_virtuels.str.contains(
            nom_recherche, case=False, na=False
        )
        resultats = df[masque]
        ...
        print(
            "✅ Modification effectuée ! Le fichier de l'équipe a bien été mis à jour."
        )
        return True
