...
class AnalysePhysiologique:
    ...
    def generer_graphique_taille(self) -> None:
        ...
        plt.title(
            f"Distribution des tailles - {self.sport.name.capitalize()}",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def generer_heatmap_taille_victoire(self, matchs: list[Any]) -> None:
        ...
        heatmap = plt.hexbin(
            tailles_valides, win_rates_valides, gridsize=15, cmap="YlOrRd", mincnt=1
        )
        ...
        plt.title(
            f"Heatmap : Taille vs Win Rate - {self.sport.name.capitalize()}",
            fontsize=14,
            fontweight="bold",
        )
        plt.xlabel("Taille (cm)", fontweight="bold")
