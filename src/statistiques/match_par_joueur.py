...
class PlayerStatsCalculator:
    ...
    def obtenir_bilan(self, nom_joueur: str) -> dict | str:
        ...
        for match in self.matchs:
            is_home = any(nom_recherche in str(p).lower() for p in getattr(match, "list_home_player", []))
            is_away = any(nom_recherche in str(p).lower() for p in getattr(match, "list_away_player", []))
    ...
    def _analyser_victoire(self, match, is_home, is_away):
        ...
        if self.sport_name == "tennis":
            ...
            team_won = (is_home and winner == blue_team) or (is_away and winner != blue_team)
