...
class MatchFormatter:
    ...
    def __init__(self, match: Any, liste_equipes: List[Any] | None = None) -> None:  
        ...
        self.nom_home = self._trouver_nom_equipe(
            noms_possibles=["home_team_name", "team_home", "team_blue_name", "home_team"
                            , "team_name_home"], 
            ids_possibles=["home_team_api_id", "team_id_home"],
            nom_par_defaut="Équipe Domicile"
        )
        self.nom_away = self._trouver_nom_equipe(
            noms_possibles=["away_team_name", "team_away", "team_red_name", "away_team"
                            , "team_name_away"], 
            ids_possibles=["away_team_api_id", "team_id_away"],
            nom_par_defaut="Équipe Extérieure"
        )

    def _trouver_nom_equipe(self, noms_possibles: list[str], ids_possibles: list[str], 
                            nom_par_defaut: str) -> str:
        ...
        if id_equipe:
            for equipe in self.liste_equipes:
                e_id = str(getattr(equipe, "id", getattr(equipe, "team_api_id"
                                            , getattr(equipe, "team_id", "")))).strip()
                if e_id == id_equipe:
                    return str(getattr(equipe, "name", getattr(equipe, "team_long_name"
                                                            , nom_par_defaut))).strip()
    ...
    def _recuperer_score(self) -> str:
        ...
        if self.sport.lower() in ["football", "basketball", "volley"]:
            h_score = getattr(self.match, "pts_home", getattr(self.match, 
                        "home_team_score", getattr(self.match, "home_team_goal", "?")))
            a_score = getattr(self.match, "pts_away", getattr(self.match, 
                        "away_team_score", getattr(self.match, "away_team_goal", "?")))
    ...
    def generer_texte_console(self) -> str:
        ...
        texte += f"\n\n🏠 {self.nom_home.upper()} :\n> {self._formater_joueurs
                                        (getattr(self.match, 'list_home_player', []))}"
        texte += f"\n\n✈️ {self.nom_away.upper()} :\n> {self._formater_joueurs
                                        (getattr(self.match, 'list_away_player', []))}"
        texte += "\n" + "="*50
