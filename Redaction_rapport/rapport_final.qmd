...
\rule{\textwidth}{0.5mm}\\
\vspace{0.8cm}
{\huge\bfseries Traitement et Visualisation de Données Sportives}\\
\vspace{0.8cm}
\rule{\textwidth}{0.5mm}\\
...
Pour ce faire, nous devions manipuler divers jeux de données bruts. Tout l'enjeu a été de concevoir un système capable d'ingérer, de nettoyer et de structurer ces informations afin de les rendre exploitables par notre application, quel que soit le contexte sportif.
...
Ce projet a été l’occasion de comprendre plus généralement la notion de programmation orientée objet au travers de la nécessité de créer toutes nos statistiques en partant d’une base vierge. La période sans jeux de données, bien que compliquée, a permis de tenter de généraliser au maximum nos idées afin qu’elles puissent ensuite être appliquées aux sports dont les données nous ont été fournies, assurant de développer notre vision d’ensemble. L’utilisation de GitHub à quatre a également été un défi majeur car c’était la première fois que nous devions réellement partager un code à plusieurs. L’organisation a donc été un point important afin de garantir que chacun puisse fusionner correctement son code.
